...
<html>
<head>
<title>Entertainment: Language Database</title>
<meta charset="UTF-8">
...
$sql='SELECT * FROM `language` LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    elseif($_GET['id']=='last_update') 
    {
        ...
        $sql = "SELECT * FROM `language` " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM `language` WHERE (language_id LIKE '" . $query . "%') OR (name LIKE '". $query ."%')";
}
...
{	 
	 ...
	 VALUES ('$language_id','$name',NOW())";
      ...
      ?>
        ...
        echo '<meta http-equiv="refresh" content="2;url=content_of_language.php" />';
...
{	 
    ...
    $sql ="UPDATE `language` SET last_update=NOW() WHERE language_id=$language_id";
		if (mysqli_query($db, $sql) === TRUE) {
      $status = "Query has been updated";
        $status_code = "success";
        ?>
...
<div class="header">
  ...
  <form class="header_button" action="content_of_language.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_language.php" method="post">
    ...
    <div class="container-update">
        ...
        <form action="content_of_language.php" method="post">
            <div class ="data-update">
                <label>Enter Language ID (to update):</label>
...
<thead>
  <tr>
    <th><a href="content_of_language.php?id=<?php echo 'language_id';?>&action=<?php echo $action;?>">Language_Id</a></th>
    <th><a href="content_of_language.php?id=<?php echo 'name';?>&action=<?php echo $action;?>">Name</a></th>
    <th><a href="content_of_language.php?id=<?php echo 'last_update';?>&action=<?php echo $action;?>">Last_Update</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_language.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_language.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
