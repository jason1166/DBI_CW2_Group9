...
<html>
<head>
<title>Entertainment: Film Text Database</title>
<meta charset="UTF-8">
...
$sql='SELECT * FROM film_text LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    elseif($_GET['id']=='title') 
    {
        $id = "title";
    }
    elseif($_GET['id']=='description') 
    {
        $id="description";
    }
        $where = " ORDER BY  $id $action ";
        $sql = "SELECT * FROM film_text " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM film_text WHERE (film_id LIKE '" . $query . "%') OR (title LIKE '". $query ."%') OR (`description` LIKE '". $query ."%')";
}
...
{	 
	 ...
	 if (mysqli_query($db, $sql)) {
        ...
        ?>
          ...
          echo '<meta http-equiv="refresh" content="2;url=content_of_film_text.php" />';
        ...
        ?>
          ...
          echo '<meta http-equiv="refresh" content="3;url=content_of_film_text.php" />';
      }
}
...
{	 
	...
	if(!empty($title)){
		$sql ="UPDATE film_text SET title='$title' WHERE film_id=$film_id";
    ...
    if(!empty($description)){
		$sql ="UPDATE film_text SET `description`='$description' WHERE film_id=$film_id";
        if (mysqli_query($db, $sql) === TRUE) {
            $status = "Query has been updated";
            $status_code = "success";
            ?>
...
<div class="header">
  ...
  <form class="header_button" action="content_of_film_text.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_film_text.php" method="post">
            <div class ="data">
                <label>Film ID:</label>
...
<thead>
  <tr>
    <th><a href="content_of_film_text.php?id=<?php echo 'film_id';?>&action=<?php echo $action;?>">Film ID</a></th>
    <th><a href="content_of_film_text.php?id=<?php echo 'title';?>&action=<?php echo $action;?>">Title Name</a></th>
    <th colspan="3"><a href="content_of_film_text.php?id=<?php echo 'description';?>&action=<?php echo $action;?>">Description</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_film_text.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_film_text.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
