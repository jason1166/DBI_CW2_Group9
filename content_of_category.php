...
<html>
<head>
<title>9 Warriors Entertainment: Category Table</title>
<meta charset="UTF-8">
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    elseif($_GET['id']=='last_update') 
    {
        ...
        $sql = "SELECT * FROM category " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM category WHERE (category_id LIKE '" . $query . "%') OR (`name` LIKE '". $query ."%')";
}
...
{	 
	 ...
	 if (mysqli_query($db, $sql)) {
        ...
        ?>
          ...
          echo '<meta http-equiv="refresh" content="3;url=content_of_category.php" />';
...
{	 
    ...
    $sql ="UPDATE category SET last_update = NOW() WHERE category_id=$category_id";
    if (mysqli_query($db, $sql) === TRUE) {
        $status = "Query has been updated";
        $status_code = "success";
        ?>
...
<div class="header">
  ...
  <form class="header_button" action="content_of_category.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_category.php" method="post">
            <div class ="data">
    ...
    <div class="container-update">
        ...
        <form action="content_of_category.php" method="post">
            <div class ="data-update">
                <label>Enter Category ID (to update):</label>
...
<thead>
  <tr>
    <th><a href="content_of_category.php?id=<?php echo 'category_id';?>&action=<?php echo $action;?>">Category ID</a></th>
    <th><a href="content_of_category.php?id=<?php echo 'name';?>&action=<?php echo $action;?>">Name</a></th>
    <th><a href="content_of_category.php?id=<?php echo 'last_update';?>&action=<?php echo $action;?>">Last Update</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_category.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_category.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
