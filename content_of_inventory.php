...
<html>
<head>
<title>Entertainment: Inventory Database</title>
<meta charset="UTF-8">
...
$sql='SELECT * FROM inventory LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    if($_GET['id']=='inventory_id') 
    {
        $id = "inventory_id";
    }
    elseif($_GET['id']=='film_id') 
    {
        $id = "film_id";
    }
    ...
    elseif($_GET['id']=='last_update') 
    {
        ...
        $sql = "SELECT * FROM inventory " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM inventory WHERE (inventory_id LIKE '" . $query . "%') OR (film_id LIKE '". $query ."%') OR (store_id LIKE '". $query ."%') ";
}
...
{	 
   ...
   $store_id = $_POST['store_id'];
    ...
    ?>
      ...
      echo '<meta http-equiv="refresh" content="3;url=content_of_inventory.php" />';
}
else{
    ?>
      <script>
...
{	 
   ...
   $status_code = "success";
    ...
    if(!empty($film_id)){
		$sql ="UPDATE inventory SET film_id='$film_id' WHERE inventory_id = $inventory_id";
    ...
    if(!empty($store_id)){
		$sql ="UPDATE inventory SET store_id='$store_id' WHERE inventory_id = $inventory_id";
    if (mysqli_query($db, $sql) === TRUE) {
    ...
    $sql ="UPDATE inventory SET last_update=NOW() WHERE INVENTORY_id=$inventory_id";
		if (mysqli_query($db, $sql) === TRUE) {
      $status = "Query has been updated";
        $status_code = "success";
        ?>
...
<div class="header">
  ...
  <form class="header_button" action="content_of_inventory.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_inventory.php" method="post">
    ...
    <div class="container-update">
        ...
        <form action="content_of_inventory.php" method="post">
            <div class ="data-update">
                <label>Enter Inventory ID (to update):</label>
                <input type ="text" name="inventory_id" required>
            </div>
            <div class ="data-update">
                <label>Change Film ID to:</label>
...
<thead>
  <tr>
    <th><a href="content_of_inventory.php?id=<?php echo 'inventory_id';?>&action=<?php echo $action;?>">Inventory ID</a></th>
    <th><a href="content_of_inventory.php?id=<?php echo 'film_id';?>&action=<?php echo $action;?>">Film ID</a></th>
    <th><a href="content_of_inventory.php?id=<?php echo 'Store_id';?>&action=<?php echo $action;?>">Store ID</a></th>
    <th><a href="content_of_inventory.php?id=<?php echo 'last_update';?>&action=<?php echo $action;?>">Last_Update</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_inventory.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_inventory.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
