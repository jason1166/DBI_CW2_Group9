...
<html>
<head>
<title>Entertainment: City Database</title>
<meta charset="UTF-8">
...
$sql='SELECT * FROM city LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    elseif($_GET['id']=='last_update') 
    {
        ...
        $sql = "SELECT * FROM city " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM city WHERE (city_id LIKE '" . $query . "%') OR (city LIKE '". $query ."%') OR (country_id LIKE '". $query ."%')";
}
...
{	 
	 ...
	 if (mysqli_query($db, $sql)) {
        ...
        ?>
          ...
          echo '<meta http-equiv="refresh" content="2;url=content_of_city.php" />';
        ...
        ?>
          ...
          echo '<meta http-equiv="refresh" content="3;url=content_of_city.php" />';
      }
}
...
{	 
    ...
    $country_id = $_POST['country_id'];
    if(!empty($city)){
		$sql ="UPDATE city SET city = '$city' WHERE city_id = $city_id";
    ...
    if(!empty($country_id)){
		$sql ="UPDATE city SET country_id = $country_id WHERE city_id = $city_id";
        if (mysqli_query($db, $sql) === TRUE) {
    ...
    $sql ="UPDATE city SET last_update=NOW() WHERE city_id = $city_id";
    if (mysqli_query($db, $sql) === TRUE) {
...
<div class="header">
  ...
  <form class="header_button" action="content_of_city.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_city.php" method="post">
        <!--<form action="content_of_country.php" method="post">-->
            ...
            <div class ="data">
                <label>City:</label>
                <input type ="text" name="city" pattern="[A-Za-z]{1,}" required>
            </div>
            <div class ="data">
    ...
    <div class="container-update">
        ...
        <form action="content_of_city.php" method="post">
            <div class ="data-update">
                <label>Enter City ID (to update):</label>
                <input type ="text" name="city_id" required>
            </div>
            <div class ="data-update">
                <label>Change City to:</label>
                <input type ="text" name="city" pattern="[A-Za-z]{0,}">
            </div>
            <div class ="data-update">
                <label>Change Country ID to:</label>
...
<thead>
  <tr>
    <th><a href="content_of_city.php?id=<?php echo 'city_id';?>&action=<?php echo $action;?>">City ID</a></th>
    <th><a href="content_of_city.php?id=<?php echo 'city';?>&action=<?php echo $action;?>">City</a></th>
    <th><a href="content_of_city.php?id=<?php echo 'country_id';?>&action=<?php echo $action;?>">Country ID</a></th>
    <th><a href="content_of_city.php?id=<?php echo 'last_update';?>&action=<?php echo $action;?>">Last Update</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_city.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_city.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
