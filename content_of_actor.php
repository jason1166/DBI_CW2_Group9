...
<html>
<head>
<title>9 Warriors Entertainment: Actor Table</title>
<meta charset="UTF-8">
...
$sql='SELECT * FROM actor LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    elseif($_GET['id']=='last_update') 
    {
        ...
        $sql = "SELECT * FROM actor " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM actor WHERE (actor_id LIKE '" . $query . "%') OR (first_name LIKE '". $query ."%') OR (last_name LIKE '". $query ."%')";
}
...
{	 
	 ...
	 VALUES ('$actor_id','$first_name','$last_name',NOW())";
	 if (mysqli_query($db, $sql)) {
        ...
        ?>
          ...
          echo '<meta http-equiv="refresh" content="3;url=content_of_actor.php" />';
...
{	 
	...
	if(!empty($first_name)){
		$sql ="UPDATE actor SET first_name='$first_name' WHERE actor_id=$actor_id";
    ...
    $sql ="UPDATE actor SET last_update=NOW() WHERE actor_id=$actor_id";
    if (mysqli_query($db, $sql) === TRUE) {
        $status = "Query has been updated";
          $status_code = "success";
          ?>
...
<div class="header">
  ...
  <form class="header_button" action="content_of_actor.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_actor.php" method="post">
            <div class ="data">
...
<thead>
  <tr>
    <th><a href="content_of_actor.php?id=<?php echo 'actor_id';?>&action=<?php echo $action;?>">Actor ID</a></th>
    <th><a href="content_of_actor.php?id=<?php echo 'first_name';?>&action=<?php echo $action;?>">First Name</a></th>
    <th><a href="content_of_actor.php?id=<?php echo 'last_name';?>&action=<?php echo $action;?>">Last Name</a></th>
    <th><a href="content_of_actor.php?id=<?php echo 'last_update';?>&action=<?php echo $action;?>">Last Update</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_actor.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_actor.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
