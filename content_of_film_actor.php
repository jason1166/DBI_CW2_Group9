...
<html>
<head>
<title>9 Warriors Entertainment: Film Actor Table</title>
<meta charset="UTF-8">
...
$sql='SELECT * FROM film_actor LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
...
if(isset($_GET["id"]))
{
  ...
  $action = $_GET["action"]; 
    ...
    elseif($_GET['id']=='last_update') 
    {
        ...
        $sql = "SELECT * FROM film_actor " . $where . ' LIMIT '. $this_page_first_result . ',' .  $results_per_page;
}
...
if(isset($_GET["search"]))
{
  ...
  $sql = 
  "SELECT * FROM film_actor WHERE (actor_id LIKE '" . $query . "%') OR (film_id LIKE '". $query ."%')";
}
...
{	 
    ...
    ?>
      ...
      echo '<meta http-equiv="refresh" content="3;url=content_of_film_actor.php" />';
...
{	 
	 $actor_id = $_POST['actor_id'];
	 $film_id = $_POST['film_id'];
   $film_id_original=$_POST['film_id_original'];

   if(!empty($film_id)){
		$sql ="UPDATE `film_actor` SET film_id='$film_id' WHERE actor_id = $actor_id AND film_id='$film_id_original'";
    ...
    $sql ="UPDATE `film_actor` SET last_update = NOW() WHERE actor_id = $actor_id AND film_id='$film_id'";
		if (mysqli_query($db, $sql) === TRUE) {
        $status = "Query has been updated";
        $status_code = "success";
        ?>
...
<div class="header">
  ...
  <form class="header_button" action="content_of_film_actor.php" method="GET">
  <div id="stripe">
...
<div>
    ...
    <div class="container">
        ...
        <form action="content_of_film_actor.php" method="post">
            <div class ="data">
    ...
    <div class="container-update">
        ...
        <form action="content_of_film_actor.php" method="post">
            <div class ="data-update">
                <label>Enter Actor ID (to update):</label>
                <input type ="text" name="actor_id" required>
            </div>
            <div class ="data-update">
                <label>Original Film ID to:</label>
                <input type ="text" name="film_id_original" required>
            </div>
            <div class ="data-update">
...
<thead>
  <tr>
    <th><a href="content_of_film_actor.php?id=<?php echo 'actor_id';?>&action=<?php echo $action;?>">Actor ID</a></th>
    <th><a href="content_of_film_actor.php?id=<?php echo 'film_id';?>&action=<?php echo $action;?>">Film ID</a></th>
    <th><a href="content_of_film_actor.php?id=<?php echo 'last_update';?>&action=<?php echo $action;?>">Last Update</a></th>
  </tr>
</thead>
...
<?php echo '<option value="content_of_film_actor.php">Page: ' . $current_page . '</option>';?>
<?php for ($page=1; $page<=$number_of_pages; $page++) {?>
      <?php echo '<option value="content_of_film_actor.php?page=' . $page . '">' . $page . '</a></option>'; ?>
<?php }?>
</select>
